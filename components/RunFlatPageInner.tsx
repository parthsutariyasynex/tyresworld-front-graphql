...
const PAGE_SIZE = 12;
const CATEGORY_URL_KEY = "run-flat-tires";
...
function StickySearch({
  ...
  useEffect(() => {
    fetch(`/api/search-options?store=${store}`)
      .then(r => r.json())
  ...
  const sel =
    "flex-1 bg-white/15 text-white text-sm font-semibold rounded-xl px-4 py-3 outline-none appearance-none cursor-pointer border border-white/20 hover:border-white/40 transition-colors";
}

/* ════════════════════════════════════════════════════════════════
   MAIN — must be inside <Suspense> (uses useSearchParams)
════════════════════════════════════════════════════════════════ */
export default function RunFlatPageInner({ locale }: { locale: Locale }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const dir = locale === "ar" ? "rtl" : "ltr";
  const store = storeCode(locale);
  const basePath = `/${locale}/run-flat-tires`;

  const sort = searchParams.get("product_list_order") ?? searchParams.get("sort") ?? "";
  const page = Math.max(1, Number(searchParams.get("page") ?? 1));

  /* ── Category + Products (single combined request) ───────────── */
  ...
  useEffect(() => {
    ...
    /* decode HTML entities from Magento PageBuilder */
