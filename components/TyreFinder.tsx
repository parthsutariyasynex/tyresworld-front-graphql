...
/* ────────────────────────────────────────────────────────────────── */
export default function TyreFinder({ locale: localeProp, categoryUid }: TyreFinderProps) {
  ...
  const handleSizeSearch = (e: React.FormEvent) => {
    ...
    router.push(`${tyresBase}?${appendCategory(buildFilterParams(
  ...
  /* ── size modal helpers ────────────────────────────────────────── */
  const openSize = (step: SizeStep) => { setSizeStep(step); setSizeOpen(true); };
  ...
  const openVeh = (step: VehStep) => { setVehStep(step); setVehQuery(""); setVehOpen(true); };
                  ...
                  {tab === "size" && (
                    <div className="flex items-center w-full h-full">
                      ...
                      <div className="search-field-wrap">
                        <button type="button" disabled={widthLoading} onClick={() => openSize("width")} className={fieldBtn}>
      ...
      {vehOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-[760px] max-h-[85vh] bg-white rounded-[24px] shadow-2xl overflow-hidden flex flex-col border border-gray-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            {/* Options */}
            <div className={`p-6 overflow-y-auto flex-1 custom-scrollbar ${
              (selVehicle && selModel && selYear && selEngine) ? "flex flex-col justify-center" : ""
            }`}>
              {depLoading ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <Loader2 size={32} className="animate-spin text-[#ed1c24]" />
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                              {engs.map((o) => (
                                <button
                                  ...
                                  className={`border py-3.5 px-2 rounded-xl text-center text-sm font-bold transition-all duration-150 hover:scale-[1.02] active:scale-[0.98] ${
                                    selEngine === o.value
                                      ? "bg-black text-white border-black"
                                      : "bg-white text-ink border-gray-200 hover:border-black hover:bg-gray-50"
                                  }`}
